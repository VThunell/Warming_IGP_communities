...
ggplot() +
  ...
  scale_color_manual(values = c("#000000", "#999999", "#000000", "#000000"),
                     name = "State", labels = c("PCR", "CR", "Unst. PCR", "Limit cycles"), drop = FALSE) +
  scale_x_continuous(expression(paste(Delta,italic("T "),"(relative to ",italic("T")["0"],")"," [K]")),
...
defx %>% 
  ...
  ggplot(.) + 
   ...
   ylab(expression(paste("Predator resource preference (", italic("\u03B2"), ")"))) +   
   xlab(expression(paste(Delta,italic("T "),"(relative to ",italic("T")["0"],")"," [K]"))) +
